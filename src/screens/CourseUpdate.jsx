...
export const CourseUpdate = () => {
	...
	const [updateCourse, { error: errorCourse }] = useUpdateCourseMutation();
	...
	useEffect(() => {
		if (course?.result) {
			setTitle(course.result.title);
			setDescription(course.result.description);
			setDuration(course.result.duration);
			let authorsWithData = course.result.authors.map((id) => {
				if (authorsData.result)
					return authorsData.result?.find((authorWithName) => {
						return authorWithName.id === id;
					});
				return null;
			});
					...
					<div className='add-button'>
						<Button
							...
							onClick={async () => {
								...
								let response = await updateCourse({
									id: courseId,
									body: {
										title: title,
